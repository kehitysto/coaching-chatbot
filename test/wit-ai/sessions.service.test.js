...
describe('Sessions service', function() {
    ...
    describe('#read()', function() {
        before(function() {
            this.db.prototype.get = sinon.stub().callsArgWith(
                1,
                null,
                {
                    id: 'SESSION_ID',
        ...
        it('should request the session context from DynamoDB', function() {
            return expect(
                ...
                return expect(this.db.prototype.get).to.have.been.calledWith(
                    sinon.match({
                        Key: { id: 'SESSION_ID' },
    ...
    describe('#write()', function() {
        before(function() {
            this.db.prototype.put = sinon.stub().callsArg(1);
        });

        it('should return a Promise', function() {
            const ret = this.sessions.write('SESSION_ID', { key: 'value' });
        ...
        it('should post the session context to DynamoDB', function() {
            return expect(
                this.sessions.write('SESSION_ID', { key: 'value' })
            ).to.eventually.be.fulfilled.then(() => {
                return expect(this.db.prototype.put).to.have.been.calledWith(
                    sinon.match({
                        TableName: sinon.match.string,
                        Item: {
                            id: 'SESSION_ID',
